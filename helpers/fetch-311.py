#!/usr/bin/python3
"""Secure fetch-311 fetch — timeout/byte caps, atomic nofollow cache."""
...
def fail(msg):
    print(f"fetch-311: {msg}", file=sys.stderr)
    sys.exit(1)
...
def validate_cache_path(p):
    ...
    if not (p.startswith(exp) or p == os.path.join(home, ".cache", "omarchy", "yerrr", "311.json")):
...
def main():
    ...
    url = "https://data.cityofnewyork.us/resource/erm2-nwe9.json?$limit=" + str(lim) + "&$order=created_date DESC"

#!/usr/bin/python3
"""Secure fetch-dob fetch — timeout/byte caps, atomic nofollow cache."""
...
def fail(msg):
    print(f"fetch-dob: {msg}", file=sys.stderr)
    sys.exit(1)
...
def validate_cache_path(p):
    ...
    if not (p.startswith(exp) or p == os.path.join(home, ".cache", "omarchy", "yerrr", "dob.json")):
...
def main():
    ...
    url = "https://data.cityofnewyork.us/resource/ipu4-2q9a.json?$limit=" + str(lim) + "&$order=pre_filing_date%20DESC"

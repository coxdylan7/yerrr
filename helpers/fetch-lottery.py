#!/usr/bin/python3
"""Secure lottery fetch — Powerball + Take5, byte caps, atomic nofollow."""
import sys, os, json, pathlib, urllib.request, stat, re, secrets
MAX_BYTES = 512*1024
TIMEOUT = 10
def fail(msg):
    print(f"fetch-lottery: {msg}", file=sys.stderr)
    sys.exit(1)
...
def ensure_parent_secure_pinned(parent_fd, parent_path):
    ...
    if st.st_mode & (stat.S_IWGRP|stat.S_IWOTH): fail(f"parent writable: {parent_path}")
...
def fetch(url):
    ...
    with urllib.request.urlopen(req, timeout=TIMEOUT) as resp:
        ...
        if cl and int(cl)>MAX_BYTES: fail("content-length exceeds")
    ...
    for u in urls:
        try:
            ...
            if isinstance(j, list):
                for r in j:
                    r["game"] = "Powerball" if "d6yy" in u else "Take5"
                    merged.append(r)

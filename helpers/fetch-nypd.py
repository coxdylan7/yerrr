#!/usr/bin/python3
"""Secure fetch-nypd fetch — timeout/byte caps, atomic nofollow cache."""
...
def fail(msg):
    print(f"fetch-nypd: {msg}", file=sys.stderr)
    sys.exit(1)
...
def validate_cache_path(p):
    ...
    if not (p.startswith(exp) or p == os.path.join(home, ".cache", "omarchy", "yerrr", "nypd.json")):

#!/usr/bin/python3
"""Secure fetch-parking fetch — timeout/byte caps, atomic nofollow cache."""
...
def fail(msg):
    print(f"fetch-parking: {msg}", file=sys.stderr)
    sys.exit(1)
...
def validate_cache_path(p):
    ...
    if not (p.startswith(exp) or p == os.path.join(home, ".cache", "omarchy", "yerrr", "parking.json")):
...
def main():
    ...
    url = "https://data.cityofnewyork.us/resource/nc67-uf89.json?$limit=" + str(lim) + "&$order=issue_date%20DESC"
    ...
    try:
        ...
        mock = [{"summons_number":"1","zip_code":"11211","violation_code":"21"}]
